...
// Uses localStorage for client-side tracking, PostgreSQL for server-side
...
export class QRTrackingService {
  ...
  static async getAnalytics(vendorId?: string): Promise<QRAnalytics> {
    try {
      // Fetch from localStorage for client-side
      const scans = typeof window !== 'undefined'
        ? JSON.parse(localStorage.getItem('qr-scans') || '[]') as QRScan[]
        : [];
