...
export async function POST(request: NextRequest) {
  try {
    ...
    // Determine target dimensions based on type or provided dimensions
    let targetWidth = 800;
    let targetHeight = 800;
    
    if (body.dimensions) {
      targetWidth = body.dimensions.width;
      targetHeight = body.dimensions.height;
    } else if (body.type === 'banner') {
      targetWidth = 1200;
      targetHeight = 400;
    } else if (body.type === 'logo' || body.type === 'product') {
      targetWidth = 800;
      targetHeight = 800;
    }
...
function getImageRecommendations(metadata: any): string[] {
  ...
  // Removed hardcoded square aspect ratio recommendation
  // since we now support different aspect ratios for different image types
