...
export default function VoiceFirstChat({ 
  ...
  // Mobile detection and voice chat manager
  useEffect(() => {
    // Check if mobile
    const checkMobile = () => {
    ...
    // Subscribe to voice chat manager
    const unsubscribe = voiceChatManager.subscribe((shouldOpen) => {
      console.log('🎤 Voice chat state changed:', shouldOpen);
      if (shouldOpen && !isOpen) {
        setIsOpen(true);
        // Reset the manager state after opening
        setTimeout(() => voiceChatManager.close(), 100);
      }
    });
    
    // Also listen for custom event (backward compatibility)
    const handleOpenVoiceChat = () => {
      console.log('🎤 Voice chat event received!');
      setIsOpen(true);
    };
    
    window.addEventListener('open-voice-chat', handleOpenVoiceChat);
    console.log('🎤 Voice chat listener registered');
  ...
  return (
    <>
      {/* Floating Voice Button - Brand Red (Desktop Only) */}
