...
import { ShoppingBag, Store, ArrowRight, Users, Leaf, Heart } from 'lucide-react';
    ...
    >
      ...
      >
        ...
        <p className="mt-3 text-base sm:text-lg text-[#3a3a1d]/70 max-w-2xl mx-auto">
          {language === 'he'
            ? 'בין אם אתם קונים או מוכרים, כפר מרקט מחבר אתכם לקהילה'
            : 'Whether you shop or sell, KFAR Marketplace connects you to the Village'}
        </p>
      </motion.div>

      <motion.div
        className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8"
        variants={containerVariants}
        initial="hidden"
        animate={isInView ? 'visible' : 'hidden'}
      >
        {/* Customer CTA - Shop the Village */}
          ...
          <Link href="/vendors" className="block cursor-pointer">
            ...
            >
              ...
              <motion.div
                ...
                className={`absolute top-4 sm:top-6 ${isRTL ? 'left-4 sm:left-6' : 'right-4 sm:right-6'} bg-white/95 backdrop-blur-sm rounded-xl px-3 py-2 sm:px-4 sm:py-3 shadow-lg`}
              >
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-xs text-[#3a3a1d]/60 font-medium">
              ...
              <div className={`absolute inset-0 flex flex-col justify-end p-6 sm:p-8 lg:p-10`}>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {[
                    ...
                    <span
                      key={i}
                      className="inline-flex items-center gap-1.5 bg-white/15 backdrop-blur-sm text-white text-xs font-medium px-3 py-1.5 rounded-full border border-white/20"
                    >
                ...
                {/* CTA Button */}
                <div>
                  <span className="inline-flex items-center gap-2.5 bg-white text-[#478c0b] font-semibold text-sm sm:text-base px-6 sm:px-7 py-3 sm:py-3.5 rounded-xl shadow-lg group-hover:shadow-xl transition-all duration-300 group-hover:bg-[#478c0b] group-hover:text-white cursor-pointer">
        ...
        <motion.div variants={cardVariants} className="group">
          <Link href="/become-a-vendor" className="block cursor-pointer">
            ...
            >
              ...
              {/* Gradient Overlay - slightly warmer for vendor */}
              ...
              <div className={`absolute inset-0 flex flex-col justify-end p-6 sm:p-8 lg:p-10`}>
                ...
                <p className="text-white/80 text-sm sm:text-base leading-relaxed mb-5 max-w-md">
                  {language === 'he'
                    ? 'הצטרפו למרקטפלייס הקהילתי שלנו — הגיעו ל-500+ משפחות מקומיות ללא עמלות'
                    : 'Join our community marketplace — reach 500+ local families with zero commission fees'}
                </p>

                {/* CTA Button */}
                <div>
                  <span className="inline-flex items-center gap-2.5 bg-[#C4A265] text-white font-semibold text-sm sm:text-base px-6 sm:px-7 py-3 sm:py-3.5 rounded-xl shadow-lg group-hover:shadow-xl transition-all duration-300 group-hover:bg-white group-hover:text-[#2D5A27] cursor-pointer">
                    <Store className="w-4 h-4 sm:w-5 sm:h-5 stroke-[1.5]" />
